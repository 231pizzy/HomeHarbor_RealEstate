...
export default function Profile() {
  ...
  const handleShowListings = async () => {
    try {
      ...
      if (showListings) {
        // If listings are already shown, close them by resetting user listings
        setUserListings([]);
      ...
      {showConfirmation && (
        <div className="confirmation-dialog text-center text-red-700 mt-5">
          ...
          <div className="flex justify-center gap-3 mt-3">
            <button
              className="text-red-700 px-4 py-2 border border-red-700 rounded hover:bg-red-700 hover:text-white"
              onClick={confirmDeleteUser}
            >
              Yes
            </button>
            <button
              className="text-green-700 px-4 py-2 border border-green-700 rounded hover:bg-green-700 hover:text-white"
